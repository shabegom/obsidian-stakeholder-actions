...
export class TodoParser {
  ...
  constructor(props: TodoItemIndexProps) { this.props = props; }

  async parseTasks(filePath: string,
                   fileContents: string): Promise<TodoItem[]> {
    const pattern = /(-|\*) \[(\s|x)?\]\s(.*)/g;
    return [...fileContents.matchAll(pattern) ].map(
        (task) => this.parseTask(filePath, task));
  }
  ...
  private parseTask(filePath: string, entry: RegExpMatchArray): TodoItem {
    ...
    if (dateMatches != null) {
      if (dateMatches.length == 4) {
        actionDate =
            new Date(parseInt(dateMatches[1]), parseInt(dateMatches[2]) - 1,
                     parseInt(dateMatches[3]), 0, 0, 0, 0);
    ...
    const misc = miscMatches != null
                     ? miscMatches.length == 2 ? miscMatches[1] : ""
                     : "";
