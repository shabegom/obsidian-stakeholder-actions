...
export default class ActionTrackerPlugin extends Plugin {
  ...
  private getTodoItemIndexProps(): TodoItemIndexProps {
    return {
      ...
      discussWithRegexp :
          new RegExp(this.getSettingValue('discussWithRegexpString')),
      waitingForRegexp :
          new RegExp(this.getSettingValue('waitingForRegexpString')),
      promisedToRegexp :
          new RegExp(this.getSettingValue('promisedToRegexpString')),
      somedayMaybeRegexp :
          new RegExp(this.getSettingValue('somedayMaybeRegexpString')),
    };
  }
  ...
  async onload(): Promise<void> {
    ...
    if (this.app.workspace.layoutReady) {
      ...
      this.registerEvent(this.app.workspace.on(
          'layout-ready', async () => await this.prepareIndex()));
    }
  }

  onunload(): void {
    this.app.workspace.getLeavesOfType(VIEW_TYPE_TODO)
  ...
  async prepareIndex(): Promise<void> { await this.todoIndex.initialize(); }
  ...
  getSettingValue<K extends keyof ActionTrackerSettings>(setting: K):
